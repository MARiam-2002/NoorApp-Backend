...
`;
function walkRouteDirs(dir, ext, results) {
    let entries;
    try {
    ...
    for (const e of entries) {
        ...
        if (e.isDirectory() && !e.isSymbolicLink()) {
            walkRouteDirs(full, ext, results);
            continue;
        }
        if (!e.isFile() || !full.endsWith(ext))
            continue;
        const normalized = full.split(node_path_1.default.sep).join('/');
        if (!results.includes(normalized))
            results.push(normalized);
    }
}
function resolveSwaggerFiles() {
    const root = node_process_1.default.cwd();
    const results = [];
    try {
        // First, try to find TypeScript source files (development mode)
        if (node_fs_1.default.existsSync(node_path_1.default.join(root, 'src', 'routes'))) {
            walkRouteDirs(node_path_1.default.join(root, 'src', 'routes'), '.ts', results);
            logger_1.logger.debug('[Swagger] Source TS files matched', { count: results.length });
        }
        // If no TS files found or in production, look for compiled JS files
        if (results.length === 0 && node_fs_1.default.existsSync(node_path_1.default.join(root, 'dist', 'routes'))) {
            walkRouteDirs(node_path_1.default.join(root, 'dist', 'routes'), '.js', results);
            logger_1.logger.debug('[Swagger] Compiled JS files matched', { count: results.length });
        }
        if (results.length > 0) {
            logger_1.logger.info('[Swagger] Found route files', { count: results.length, files: results.slice(0, 3) });
            return results;
        }
        logger_1.logger.warn('[Swagger] No JSDoc route files found - returning empty spec paths');
        return [];
    }
    catch (err) {
        logger_1.logger.error('[Swagger] resolveSwaggerFiles failed', { err });
        return [];
    }
}
function buildSwaggerSpec() {
    const apiBasePath = (0, config_1.getApiBasePath)();
    const definition = {
        openapi: '3.1.0',
        info: {
            title: config_1.env.SWAGGER_TITLE,
            version: config_1.env.SWAGGER_VERSION,
            description: config_1.env.SWAGGER_DESCRIPTION,
    ...
    try {
        const spec = (0, swagger_jsdoc_1.default)({ definition, apis });
        logger_1.logger.info('[Swagger] Spec built', { routes: Object.keys(spec.paths || {}).length });
        return spec;
    }
    catch (err) {
        logger_1.logger.warn('[Swagger] Bulk parse failed', { err: err?.message });
        return { ...definition, paths: {} };
    }
}
exports.swaggerSpec = buildSwaggerSpec();
function setupSwagger(app) {
    if (!config_1.env.SWAGGER_ENABLED)
        return;
    const docsPath = `${(0, config_1.getApiBasePath)()}/docs`;
    app.use(docsPath, swagger_ui_express_1.default.serve, swagger_ui_express_1.default.setup(exports.swaggerSpec, {
        customSiteTitle: `${config_1.env.SWAGGER_TITLE} | توثيق API`,
